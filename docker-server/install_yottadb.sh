...
echo 'Setting up local internal, unjournalled region for vermos Session global'
/usr/local/lib/yottadb/$ydbver/mumps -run ^GDE < /opt/vermos/gde.txt
/usr/local/lib/yottadb/$ydbver/mupip create -region=vermosreg
/usr/local/lib/yottadb/$ydbver/mupip set -file -nojournal /opt/vermos/sessiondb/vermos.dat
/usr/local/lib/yottadb/$ydbver/mupip set -key_size=1019 -region qewdreg

# Install and configure mgsql routines and VERMOS interface to them
